db.system.js.save({_id: "statsAdditionalData", value:
	function() {
		var count = 0;
		db.statistic.find({sessionId: {$exists: true}, sessionTime: {$exists: false}, date: {$gt: ISODate("2013-07-01")}}).forEach(
			function(stat) {
		...
		db.statistic.find({type: "second_selling", selectedSessionId: {$exists: true}, selectedTicketTypeId: {$exists: true}, selectedPrice: {$exists: false}, date: {$gt: ISODate("2013-07-01")}}).forEach(
			function(stat) {
					...
					session.availability.forEach(
						function (availability) {
							if (availability.ticketTypeId == stat.selectedTicketTypeId) {
								...
								db.statistic.update({_id: stat._id}, {$set: {selectedSessionTime: session.time, selectedPrice: priceWithBookingFee.toFixed(4), originalMovieName: session.movieName}});
							}
						}
					)
				}
